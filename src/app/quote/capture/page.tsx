        ...
        <form onSubmit={handleSubmit} className="space-y-4 bg-white p-4 rounded shadow">
          ...
          {items.map((item, idx) => (
            <div key={idx} className="border p-2 rounded space-y-2 mb-2">
              <input className="w-full border p-2 rounded" placeholder="Description" value={item.description} onChange={e => handleItemChange(idx, 'description', e.target.value)} required />
              <input className="w-full border p-2 rounded" type="number" placeholder="Quantity" value={item.quantity} onChange={e => handleItemChange(idx, 'quantity', e.target.value)} required />
              <input className="w-full border p-2 rounded" type="number" placeholder="Price" value={item.price} onChange={e => handleItemChange(idx, 'price', e.target.value)} required />
              <button type="button" className="text-red-600" onClick={() => removeItem(idx)}>Remove</button>
            </div>
          ))}

          <button type="button" className="w-full bg-gray-200 p-2 rounded" onClick={addItem}>+ Add Item</button>
